  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white dark:bg-slate-800 rounded-[24px] shadow-[0_2px_15px_-5px_rgba(0,0,0,0.05)] dark:shadow-none border border-slate-100 dark:border-slate-700 p-6 flex flex-col justify-between h-[140px] relative overflow-hidden">
          <div className="flex justify-between items-start z-10"><span className="text-[13px] font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Total Patients</span><div className="w-10 h-10 rounded-xl bg-[#ebfcf9] dark:bg-primary/20 flex items-center justify-center"><Users className="w-5 h-5 text-primary" /></div></div>
          <div className="z-10 mt-auto"><h3 className="text-[32px] font-black text-slate-900 dark:text-white leading-none">{totalPatients}</h3></div>
        </div>
        <div className="bg-white dark:bg-slate-800 rounded-[24px] shadow-[0_2px_15px_-5px_rgba(0,0,0,0.05)] dark:shadow-none border border-slate-100 dark:border-slate-700 p-6 flex flex-col justify-between h-[140px] relative overflow-hidden">
          <div className="flex justify-between items-start z-10"><span className="text-[13px] font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-widest">New This Week</span><div className="w-10 h-10 rounded-xl bg-[#eff6ff] dark:bg-blue-900/20 flex items-center justify-center"><UserPlus className="w-5 h-5 text-blue-500" /></div></div>
          <div className="z-10 mt-auto"><h3 className="text-[32px] font-black text-slate-900 dark:text-white leading-none">{stats?.newThisWeek || 0}</h3></div>
        </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-[24px] shadow-[0_2px_15px_-5px_rgba(0,0,0,0.05)] dark:shadow-none border border-slate-100 dark:border-slate-700 overflow-hidden">
        <div className="px-6 md:px-8 py-6 flex flex-col xl:flex-row xl:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/50">
          <div className="relative w-full xl:max-w-xs"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 w-4 h-4" /><input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search by name, phone, condition..." className="w-full bg-white dark:bg-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600 border border-slate-200 dark:border-slate-600 rounded-full py-2.5 pl-11 pr-4 text-[13px] font-medium text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-[#006b5f]/20 transition-all" /></div>
          <div className="flex flex-wrap gap-2 overflow-x-auto custom-scrollbar pb-2 md:pb-0">{['All Patients', 'New', 'Follow_up', 'Recovered', 'Critical'].map(filter => (<button key={filter} onClick={() => { setActiveFilter(filter); setPage(1); }} className={`px-5 py-2 rounded-full text-[13px] font-bold whitespace-nowrap transition-all ${activeFilter === filter ? 'bg-[#006b5f] text-white shadow-md shadow-[#006b5f]/20' : 'bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-600'}`}>{filter.replace('_', ' ')}</button>))}</div>
        </div>

        <div className="w-full overflow-x-auto custom-scrollbar min-h-[300px]">
          {isLoading ? (
            <div className="divide-y divide-slate-50 dark:divide-slate-700">
              {[...Array(5)].map((_, i) => (
                <div key={i} className="flex items-center gap-4 px-6 py-4">
                  <div className="h-12 w-12 rounded-full bg-slate-100 dark:bg-slate-700 animate-pulse shrink-0" />
                  ...
                  <div className="h-4 w-24 bg-slate-100 dark:bg-slate-700 rounded animate-pulse hidden md:block" />
                  <div className="h-6 w-16 bg-slate-100 dark:bg-slate-700 rounded-lg animate-pulse" />
                </div>
              ))}
            </div>
          ) : patientsData?.data?.length === 0 ? (
            <div className="flex flex-col items-center justify-center p-16 text-slate-400"><Users className="h-12 w-12 mb-4 opacity-20" /><p className="font-bold text-lg text-slate-500 dark:text-slate-400">No Patient Records</p></div>
          ) : (
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-700/50 text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.15em]"><th className="px-6 md:px-8 py-5 min-w-[250px]">Patient Name</th><th className="px-6 py-5 min-w-[150px]">Last Visit</th><th className="px-6 py-5 min-w-[150px]">Upcoming</th><th className="px-6 py-5 min-w-[200px]">Contact & Status</th><th className="px-6 py-5 w-16"></th></tr>
              </thead>
              <tbody className="divide-y divide-slate-50 dark:divide-slate-700">
                {patientsData?.data?.map((p: any) => (
                  <tr key={p.id} className="hover:bg-slate-50/70 dark:hover:bg-slate-700/30 transition-colors group">
                    <td className="px-6 md:px-8 py-5"><div className="flex items-center gap-4"><div className={`w-12 h-12 rounded-full flex items-center justify-center font-black text-lg flex-shrink-0 ${getAvatarColor(p.full_name)}`}>{(p.full_name || '?').charAt(0).toUpperCase()}</div><div><h4 className="font-extrabold text-[15px] text-slate-900 dark:text-white line-clamp-1">{p.full_name}</h4><p className="text-[12px] font-bold text-slate-400 dark:text-slate-500 mt-0.5 line-clamp-1">{p.condition || 'No conditions logged'}</p></div></div></td>
                    <td className="px-6 py-5">{p.last_visit_date ? (<><p className="text-[14px] font-bold text-slate-700 dark:text-slate-200">{format(new Date(p.last_visit_date), 'MMM dd, yyyy')}</p><p className="text-[12px] font-bold text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-1">{p.last_visit_purpose || 'General Consultation'}</p></>) : <span className="text-[14px] font-bold text-slate-300 dark:text-slate-600">-</span>}</td>
                    <td className="px-6 py-5">{p.next_appointment_date ? (<><p className="text-[14px] font-bold text-slate-900 dark:text-white">{format(new Date(p.next_appointment_date), 'MMM dd, yyyy')}</p><p className="text-[12px] font-bold text-slate-900/70 dark:text-slate-400 mt-0.5">{p.next_appointment_time?.slice(0,5)}</p></>) : <span className="text-[13px] font-bold text-slate-400 dark:text-slate-500">None Scheduled</span>}</td>
                    <td className="px-6 py-5"><div className="flex items-center justify-between"><div><p className={`text-[14px] font-bold ${p.phone ? 'text-slate-700 dark:text-slate-200' : 'text-slate-300 dark:text-slate-600'}`}>{p.phone || 'No phone'}</p><p className={`text-[12px] font-bold mt-0.5 ${p.email ? 'text-slate-400 dark:text-slate-500' : 'text-slate-300 dark:text-slate-600'}`}>{p.email || 'No email'}</p></div><div className={`px-2.5 py-1 text-[10px] font-black rounded-lg uppercase tracking-widest ${STATUS_BADGES[p.status.toLowerCase()] || STATUS_BADGES.new}`}>{p.status.replace('_', ' ')}</div></div></td>
                    <td className="px-6 py-5 text-right">
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild><button className="p-2 text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 opacity-0 group-hover:opacity-100 transition-all"><MoreVertical className="w-5 h-5"/></button></DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="w-40 rounded-xl shadow-xl border-slate-100 dark:border-slate-700 dark:bg-slate-800"><DropdownMenuItem onClick={() => openEditModal(p)} className="font-bold py-2 cursor-pointer dark:text-slate-200"><Edit2 className="w-4 h-4 mr-2 text-slate-400"/> Edit</DropdownMenuItem><DropdownMenuItem onClick={() => { if(confirm('Soft delete this patient?')) deleteMutation.mutate(p.id); }} className="font-bold py-2 cursor-pointer text-destructive focus:text-destructive"><Trash2 className="w-4 h-4 mr-2 opacity-70"/> Archive Record</DropdownMenuItem></DropdownMenuContent>
                      </DropdownMenu>
                    </td>
                  </tr>
                ))}
        ...
        <div className="px-6 md:px-8 py-5 border-t border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-4"><p className="text-[13px] font-bold text-slate-400 dark:text-slate-500">Showing {(page-1)*pageSize + 1}-{Math.min(page*pageSize, totalPatients)} of {totalPatients}</p><Select value={pageSize.toString()} onValueChange={(v) => { setPageSize(Number(v)); setPage(1); }}><SelectTrigger className="h-8 text-xs font-bold w-[90px] border-slate-200 dark:border-slate-600 dark:bg-slate-700 dark:text-white"><SelectValue /></SelectTrigger><SelectContent className="rounded-xl dark:bg-slate-800 dark:border-slate-700"><SelectItem value="10" className="text-xs font-bold dark:text-slate-200">10 / pg</SelectItem><SelectItem value="20" className="text-xs font-bold dark:text-slate-200">20 / pg</SelectItem><SelectItem value="50" className="text-xs font-bold dark:text-slate-200">50 / pg</SelectItem></SelectContent></Select></div>
          <div className="flex items-center gap-1"><button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="w-8 h-8 flex items-center justify-center text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-50 rounded-lg"><ChevronLeft className="w-4 h-4" /></button><span className="px-3 text-sm font-black text-slate-700 dark:text-white">Pg {page}</span><button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page >= totalPages} className="w-8 h-8 flex items-center justify-center text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-50 rounded-lg"><ChevronRight className="w-4 h-4" /></button></div>
        </div>
      </div>
